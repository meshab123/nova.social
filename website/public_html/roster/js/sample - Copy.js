...
function setAllPersonsColors(){
    for(i in person){
        ...
        $('#person-tag-' + person[i]['name']).css('background-color',color);
        $('#oot-tag-' + person[i]['name']).css('background-color',color);
        $('#time-tag-' + person[i]['name']).css('background-color',color);
        $('#hand-tag-' + person[i]['name']).css('background-color',handColor);
    }
}
        ...
        setInterval(function () { //1 timer for everyone
            ...
            for(var i=0; i<tempPersons.length; i++){
                report += '<tr style="font-weight:bold; font-size:20px; font-family: Roboto;">' +
                    '<td id="report-list-' + tempPersons[i].name + '">' + tempPersons[i].name + '</td>' +
                    '<td id="report-list-' + tempPersons[i].time + '">'+formatTime(tempPersons[i].time)+'</td>' +
                    '</tr>';
